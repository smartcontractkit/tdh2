...
func TestDecryptionMarshalledEncryptedSecrets(t *testing.T) {
	// This public key & private shares were generated using the Functions admin tooling: https://github.com/smartcontractkit/functions-admin-tooling/tree/main/threshold_key_manager
	...
	// This ciphertext was generated using the SecretsManager.buildEncryptedSecrets function from the Functions Toolkit: https://github.com/smartcontractkit/functions-toolkit
	...
	for i, privateKeyJsonShare := range privateKeyJsonShares {
		...
		decryptedShareJson, err := decryptedShare.Marshal()
		if err != nil {
			t.Fatalf("Marshal: %v", err)
		}
		fmt.Println("Decrypted share: ", string(decryptedShareJson))
		decryptedShares[i] = decryptedShare
	}

	decrypted, err := Aggregate(ciphertext, decryptedShares[:3], 4)
	if err != nil {
